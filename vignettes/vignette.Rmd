...
```
The lapply and the while-loop take the most time. 
...
```
As expected take the for-loops and the while-loop most of the computation time. Furthermore the sort() function is also time consuming. 
...
# system.time of the dynamic algorithm with different while loop
system.time({
...
```
We can notice a very little improvement in system.time with the second version of the while loop, in which we used the vectorization method. 
...
```
Changed the while loop, gaining a very tiny improvement of run time. 
...
Question: What performance gain could you get by parallelising brute force search? \newline
Speeding up the run time of the code.
```{r}
